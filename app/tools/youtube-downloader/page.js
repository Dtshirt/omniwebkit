...
export default function YouTubeDownloader() {
  ...
  const fetchInfo = async () => {
    ...
    try {
      const res = await fetch(`${API_V1}/tools/youtube-downloader/video-info`, {
        method: 'POST',
  ...
  const handleDownload = (type) => {
    ...
    const es = new EventSource(`${API_V1}/tools/youtube-downloader/progress/${taskId}`);
    es.onmessage = (event) => {
      try {
    ...
    const downloadUrl = `${API_V1}/tools/youtube-downloader/download?url=${encodeURIComponent(url.trim())}&type=${type}${formatParam}&task_id=${taskId}`;
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-[12px] shadow-xl overflow-hidden mb-8 transition-all duration-150">
            <div className="flex flex-col md:flex-row">
              <div className="w-full md:w-[40%] relative">
                <img src={`${API_V1}/tools/youtube-downloader/download?url=${encodeURIComponent(url)}&type=thumbnail`} alt="Thumbnail" className="w-full h-full object-cover aspect-video bg-zinc-900" />
