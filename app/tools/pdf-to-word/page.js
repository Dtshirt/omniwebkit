...
export default function PdfToWord() {
    ...
    const startPoll = (jobId) => {
        pollRef.current = setInterval(async () => {
            try {
                const res = await fetch(`${API}/pdf-to-word/status/${jobId}`);
    ...
    const download = async () => {
        if (!result) return;
        try {
            const resp = await fetch(`${API}/pdf-to-word/download/${result.jobId}`);
            ...
            fetch(`${API}/pdf-to-word/cleanup/${result.jobId}`, { method: 'DELETE' }).catch(() => {});
